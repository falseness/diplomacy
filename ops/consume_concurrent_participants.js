'use strict';
// Same measured archive inventory, differing only in the selected complete review.
...
const [prepared,out,baselineArg,targetArg]=process.argv.slice(2);
const targetId=targetArg||'TASK-209/AC2';
assert.ok(['TASK-209/AC1','TASK-209/AC2'].includes(targetId));
...
const baseline=read(baselineArg||baselineFile), candidate=read(path.join(prepared,'reviewed-crosswalk.json'));
...
assert.deepEqual(changed,[targetId]);
const target=all.find(t=>t.id===targetId), row=candidate.reviews.find(r=>r.id===target.id);
const positive=current.find(r=>r.id===target.id);
write(targetId.endsWith('AC1')?'ac1-disposition.json':'ac2-disposition.json',positive);
...
for(const kind of ['delete-packets','tamper-packets','delete-review','tamper-review']) {
 ...
 try {
  ...
  const file=path.join(dir,kind.endsWith('packets')?'recipient-packets.jsonl':(targetId.endsWith('AC1')?'ac1-independent-review.json':'ac2-independent-review.json'));
...
write('comparison.json',{pass:true,changed,priorBefore:prior.length,priorAfter:after.unresolvedPriorArchives.length,
 ...
 fullAuditReady:false,controls:controls.length,comparison:'Identical measured sources/runs and proof inputs; only the selected complete review differs.'});
console.log(`PASS actual consumer ${targetId}=covered-current prior=${prior.length}->${after.unresolvedPriorArchives.length} selfChecks=8 controls=4 G09=unresolved fullAuditReady=false`);
