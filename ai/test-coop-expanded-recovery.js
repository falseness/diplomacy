...
function run() {
 for(const count of [1,4,5,6,7,8,9,10,11,12]) {
  ...
  // Authored current mechanics board; first typed melee wave is round 4.
  e(`new DemonPortal(50,10,"melee");
  ...
  for(let round=0;round<4;round++) {
   ...
   check(`round-${round+1}-spawns`,`players[${count+1}].units.map(u=>({name:u.name,x:u.coord.x,y:u.coord.y}))`,round<3?[]:[{name:'imp',x:50,y:10}]);
  }
  ...
  check('skipped-dead-no-resize','({turn:whooseTurn,round:gameRound,size:{x:grid.arr.length,y:grid.arr[0].length}})',{turn:1,round:5,size:{x:53,y:13}});
  check('occupied-no-backlog',`players[${count+1}].units.length`,1);
  // Authored dimensions and phase marker must survive even if generation is unavailable.
  e(`gameSettings.coop.localPhase={round:5,stage:'complete'};globalThis.before=JSON.stringify(getGameObject());globalThis.visionBefore=JSON.stringify(grid.fogOfWar);loadFromJson(before)`);
  check('current-save-exact','JSON.stringify(getGameObject())===before',true);
  check('save-vision-exact','JSON.stringify(grid.fogOfWar)===visionBefore',true);
  e('globalThis.savedWave=JSON.stringify(gameSettings.coop.typedWaves);advanceCoopLocalPhase();advanceCoopLocalPhase()');
  check('committed-phase-no-replay','JSON.stringify(gameSettings.coop.typedWaves)===savedWave',true);
 ...
 console.log('PASS expanded-recovery counts=1,4,5,6,7,8,9,10,11,12 first_spawn=4 turn_order=human-only shared_vision=exact current_save=exact terminal=defeat');
}
