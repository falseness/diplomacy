...
const valley = require('./coop-valley-plan');
...
if (require.main === module) {
  ...
  try {
    for (const input of inputs) {
      ...
      const rejected=[];
      for(let side=base;side<p.side;side++) rejected.push({side,plans:valley.valleyRowPlansAtSide(h,size,side).plans.length});
      check(id+'-first-feasible',rejected.map(r=>r.plans),rejected.map(()=>0));
      ...
      check(id+'-dimensions',map.mapSize,{x:p.side,y:p.side});
      ...
      const expected={side:p.side,humanTowns:h,neutralTowns:h*{tiny:1,normal:2,big:3}[size],goldmines:h*{tiny:1,normal:2,big:3}[size],portals:6*h,portalDistance:{tiny:6,normal:10,big:14}[size]};
      ...
      maps.push({id,input,map,plan:p,rejectedSides:rejected,attemptSeeds:result.trace.map(p=>p.seed),contract,endpointResults,fronts,identical:true});
