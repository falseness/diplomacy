...
function assertExclusiveNoOverlap(map, seed) {
  ...
  function claim(coord, label) {
    assert(coord.x >= 0 && coord.y >= 0 && coord.x < 9 && coord.y < 9,
...
function assertMirroredSetup(map, seed, observed) {
  ...
  assert(map.mapSize.x === 9 && map.mapSize.y === 9,
    'advanced stage 12 must generate 9x9 maps', { seed, mapSize: map.mapSize });
  assert(map.advancedEconomyStage === 12 &&
      map.economyGenerator && map.economyGenerator.stage === 'advanced-12',
    'advanced stage 12 metadata is missing', { seed, economyGenerator: map.economyGenerator });
  assert(map.symmetry && map.symmetry.axis === 'vertical' &&
      map.economyGenerator.benchmarkSpecificAdvantage === false,
    'advanced stage 12 must document fair vertical symmetry', { seed, symmetry: map.symmetry });
  assert(left.playerType === 'AIPlayerWithEconomy' &&
      right.playerType === 'SimpleAiPlayerWithEconomy',
    'advanced stage 12 should preserve economy runtime player classes',
    { seed, players: map.players.map((player) => player.playerType) });
  assert(left.gold === right.gold,
    'advanced stage 12 starting resources are not equivalent',
    { seed, leftGold: left.gold, rightGold: right.gold });
  observed.goldValues.add(left.gold);

  assert(left.towns.length >= 1 && left.towns.length <= 3 &&
      left.towns.length === right.towns.length,
    'advanced stage 12 town counts are not mirrored', { seed, left: left.towns, right: right.towns });
  ...
  assert(JSON.stringify(normalizeList(left.towns.map((town) => {
    ...
    'advanced stage 12 town coordinates or HP are not mirrored', { seed });

  for (const town of left.towns.concat(right.towns)) {
    assert(town.x > 0 && town.y > 0 && town.x < 8 && town.y < 8,
      'advanced stage 12 town should stay away from map edges', { seed, town });
    assertHp(town, buildingMaxHp.towns,
      'advanced stage 12 town HP is outside 1..maxHp', { seed });
    observed.hpValues.add('town:' + town.hp);
  }

  assert(left.suburbs.length === right.suburbs.length,
    'advanced stage 12 suburb layout counts are not mirrored', { seed });
  ...
  for (let i = 0; i < left.suburbs.length; i += 1) {
    ...
    assert(coordKey(mirrorX(leftLayout.town, map.mapSize)) === coordKey(rightLayout.town),
      'advanced stage 12 suburb town references are not mirrored', { seed, leftLayout, rightLayout });
    assert(JSON.stringify(normalizeList(leftLayout.cells.map((cell) => mirrorX(cell, map.mapSize)))) ===
      ...
      'advanced stage 12 captured suburb cells are not mirrored', { seed, leftLayout, rightLayout });
    assert(leftLayout.cells.length >= 5 && rightLayout.cells.length >= 5,
      'advanced stage 12 should create captured cells around generated towns',
    ...
    for (const cell of leftLayout.cells.concat(rightLayout.cells)) {
      assert(cell.x >= 0 && cell.y >= 0 && cell.x < 9 && cell.y < 9,
        'advanced stage 12 captured cell is outside the map', { seed, cell });
      assert(!claimed.has(coordKey(cell)),
        'advanced stage 12 captured cell is claimed more than once',
  ...
  for (const field of ['farms', 'barracks', 'towers', 'bastions']) {
    assert(left[field].length >= 1 && left[field].length === right[field].length,
      'advanced stage 12 ' + field + ' counts are not mirrored', { seed, left: left[field], right: right[field] });
    assert(JSON.stringify(normalizeList(left[field].map((building) =>
      ...
      'advanced stage 12 ' + field + ' coordinates, town references, or HP are not mirrored', { seed });
    for (const building of left[field].concat(right[field])) {
      assertHp(building, buildingMaxHp[field],
        'advanced stage 12 ' + field + ' HP is outside 1..maxHp', { seed });
      observed.hpValues.add(field + ':' + building.hp);
    }
  }

  assert(left.units.length >= 1 && left.units.length === right.units.length,
    'advanced stage 12 unit counts are not mirrored', { seed, leftUnits: left.units, rightUnits: right.units });
  assert(JSON.stringify(normalizeUnits(left.units.map((unit) => mirrorUnit(unit, map.mapSize)))) ===
    ...
    'advanced stage 12 unit type, coordinate, or HP mirrors differ', { seed });
  for (const unit of left.units.concat(right.units)) {
    const typeName = unitTypeName(unit);
    assert(unitMaxHp[typeName],
      'advanced stage 12 generated an unsupported unit type', { seed, unit, typeName });
    assertHp(unit, unitMaxHp[typeName],
      'advanced stage 12 unit HP is outside 1..maxHp', { seed, typeName });
  ...
  assert(map.economyObjects.towns === left.towns.length + right.towns.length &&
    ...
    'advanced stage 12 economy object summary is incorrect', { seed, economyObjects: map.economyObjects });
}

const { context } = loadAiScripts();
const api = new Function('context', `return {
  generateAdvancedEconomyStage12TrainingMap: context.generateAdvancedEconomyStage12TrainingMap
};`)(context);

assert(typeof api.generateAdvancedEconomyStage12TrainingMap === 'function',
  'generateAdvancedEconomyStage12TrainingMap must be exported to the AI script context');
...
for (let seed = 14900; seed < 15020; seed += 1) {
  const map = api.generateAdvancedEconomyStage12TrainingMap({ seed });
  assertMirroredSetup(map, seed, observed);
  const repeated = api.generateAdvancedEconomyStage12TrainingMap({ seed });
  assert(normalizeMap(map) === normalizeMap(repeated),
    'advanced stage 12 map generation is not deterministic for seed ' + seed);
  layouts.add(normalizeMap(map));

  const runtime = map.start();
  assert(runtime.players[1].gold === runtime.players[2].gold,
    'advanced stage 12 runtime resources are not mirrored after start', { seed });
  assert(runtime.players[1].towns.length === map.players[1].towns.length &&
    ...
    'advanced stage 12 runtime lost configured towns', { seed });
  assert(runtime.players[1].units.length >= map.players[1].units.length &&
    ...
    'advanced stage 12 runtime lost configured units', { seed });
...
for (const count of [1, 2, 3]) {
  assert(observed.townCounts.has(count),
    'advanced stage 12 fixed-seed sample did not generate town count ' + count,
    { observedTownCounts: Array.from(observed.townCounts) });
}
assert(observed.unitTypes.size >= 3,
  'advanced stage 12 fixed-seed sample did not vary generated unit types',
  { observedUnitTypes: Array.from(observed.unitTypes) });
assert(observed.hpValues.size > 8,
  'advanced stage 12 fixed-seed sample did not vary configured HP values',
  { hpValues: Array.from(observed.hpValues) });
assert(observed.goldValues.size > 1 && layouts.size > 4,
  'advanced stage 12 fixed seeds did not vary fair resources or layouts',
