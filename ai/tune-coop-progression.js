'use strict';
// Offline only. TASK-108 owns runtime installation and save-version handling.
...
function sourceCheck(dir) {
  const identity=read(path.join(dir,'source-identity.json')).sha256;
...
function main() {
  ...
  const out=path.resolve(get('--output-dir','artifacts/TASK-107')), input=path.resolve(get('--projection-dir','artifacts/TASK-106'));
  assert.notEqual(out,input);fs.mkdirSync(out,{recursive:true});
  ...
  assert.equal(scenarios.length,12*(seeds[1]-seeds[0]+1)*3);
  const keys=new Set();
  ...
  const {createFixture}=require('./test-coop-harness'),f=createFixture(undefined,()=>{}), maps=[];
  for(const a of access.filter(a=>a.seed>=seeds[0]&&a.seed<=seeds[1])) {
    const map=f.evaluate(`JSON.parse(JSON.stringify(generateCoopGame(${a.humans},{size:'${a.size}',seed:${a.seed}})))`);
    ...
    maps.push({id:a.id,size:a.size,humans:a.humans,seed:a.seed,mapHash:a.mapHash,portals:map.portals.map(({x,y})=>({x,y}))});
    console.log(`PASS map=${a.id} portals_expected=${map.portals.length} portals_observed=${map.portals.length} human_map_hash=matched`);
  }
  assert.equal(maps.length,12*(seeds[1]-seeds[0]+1));
  ...
  fs.writeFileSync(path.join(out,'ratios.svg'),`<svg xmlns="http://www.w3.org/2000/svg" width="900" height="470" viewBox="0 0 900 470"><rect width="900" height="470" fill="white"/><text x="60" y="25">Cumulative demon / human value — typical, mean across H1–4 and seeds</text><path d="M60 50 V340 H810" fill="none" stroke="black"/><text x="5" y="60">${ymax.toFixed(2)}</text><text x="25" y="340">0</text><text x="60" y="365">Round 0</text><text x="760" y="365">${rounds}</text><polyline points="${points(Array(rounds+1).fill(target))}" stroke="black" stroke-dasharray="3 3" fill="none"/>${lines.map(s=>`<polyline points="${points(s.meanRatios)}" stroke="${colors[s.size]}" stroke-dasharray="${s.name==='before'?'7 4':'none'}" fill="none" stroke-width="2"/>`).join('')}<text x="60" y="400">Blue Tiny; green Normal; orange Big. Dashed before; solid after; dotted target 1.4.</text><text x="60" y="430">All portals available. No combat losses subtracted. Opening rounds 0–2 ratio = 0.</text></svg>\n`);
