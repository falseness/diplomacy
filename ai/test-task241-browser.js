...
const {exercise}=require('./test-task241-render');
...
async function main(){
 ...
 for(const c of cases){
  ...
  try{
   assert(Number(process.env.TASK241_STOP_AT)-Date.now()>300000,'reserve service cleanup budget');
   const result=await withServices({evidenceDir:dir,bounds:{scenarioMs:240000}},async service=>{
    ...
    write(c.id+'/declared-fixture.json',{purpose:'Authored Tiny/H2/seed1 six-category board; NOT generated geometry; local rendering fixtures declared separately in render-fixture.json',...setup});
 ...
 write('draw-order.json',{rows,pass:true});write('browser-results.json',{cases:cases.map(c=>c.id),rows,pass:true});console.log('PASS browser-network journeys=2 contexts=4 fog=off,on joins=sequential,simultaneous persistence=pass cleanup=pass');
}
