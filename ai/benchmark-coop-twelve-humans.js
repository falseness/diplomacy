...
assert(vm.constants?.DONT_CONTEXTIFY, 'workload requires Node with DONT_CONTEXTIFY support');
report.context='DONT_CONTEXTIFY with realm-native intrinsics; unmodified browser scripts';
...
// Border lines are drawing output only. The headless UI never consumes or clears
// them; retaining every BFS edge measures an artificial rendering backlog.
...
capture();
for(const round of [...Array.from({length:20},(_,i)=>i),29]){
 if(round===29){
  fs.writeFileSync(path.join(out,'round-20-state.json'),f.evaluate('JSON.stringify(getGameObject())')+'\n');
  phase('disclosed-late-setup-clock-only','gameRound=29;void 0');
  console.log('SETUP fastForward=true from_completed_round=20 setup_round=29 changed_fields=gameRound units_preserved=true no_waves_or_AI_claimed_for_skipped_rounds');
 }
...
report.ai=f.evaluate('aiPhases');assert.equal(report.ai.length,21);assert(report.ai.every(p=>p.milliseconds<60000));assert(report.rounds.at(-1).actions>0);assert(report.rounds.at(-1).demons>0);
fs.writeFileSync(path.join(out,'round-30-state.json'),f.evaluate('JSON.stringify(getGameObject())')+'\n');save();
console.log('PASS twelve-human workload dimensions=68x68 portals=36 round=30 seed=0 fastForward=true setup_from=20 setup_to=29 actual_AI_phases=21 shared_vision=identical watchdog_ms=60000');
