  ...
  function check(label) {
    ...
    const observed = fixture.evaluate(`(() => {
      const row = entity => ({id: ledgerObjects.get(entity) || '<unregistered>',
        kind: externalProduction.includes(entity) ? 'production' : entity.isUnit ? 'unit' : entity.isDemonPortal ? 'portal' : entity.name === 'goldmine' ? 'goldmine' : entity.isNature ? 'nature' : entity.isExternal ? 'building' : 'town', name: entity.name, owner: entity.playerColor,
      ...
      for (const entity of externalProduction.filter(e => !e.killed)) {
        ownership.push(row(entity));
        if (grid.getBuilding(entity.coord) !== entity) problems.push('production reference');
      }
