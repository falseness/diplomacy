...
const command = (program,args) => {
  ...
  const r = spawnSync(program,args,{cwd:ROOT,encoding:'utf8',maxBuffer:32*1024*1024,
    env:{...process.env,TASK240_STOP_AT:String(stopAt)},timeout:Math.max(1,stopAt-Date.now()),killSignal:'SIGKILL'});
...
const browserCases=require('./test-task240-browser').cases.map(c=>c.id);
const categories=['melee','ranged','siege','heavy','support','chaos'];
const cases=[...require('./test-task240-source').cases,'browser-simultaneous-fog-true/committed-upgrade','browser-simultaneous-fog-true/persisted-wave'];
for(const id of browserCases){
 ...
 for(const category of categories)for(const suffix of ['stats','back','reselection','no-mutation'])cases.push(id+'/'+category+'/'+suffix);
}
write('verification-plan.json',{estimateMs:600000,targetMs:2700000,stopWorkMs:3300000,budgetMs:3600000,
 cases,tiers:['real Chromium stats/back/reselection mouse clicks','real HTTPS/Socket.IO/MongoDB','source and on-disk evidence audit'],
 fixtures:'Two authored Tiny/H2/seed1 boards, sequential fog off and simultaneous fog on; human scouts reveal the six selected portals. Second board starts at declared round 3 and commits through wave 4. No runtime gameplay mutation.',
 coverage:'Six categories: actual stats/back/reselection mouse clicks, frame persistence, static combat values, bombard targeting, no entity/gold/undo/command mutation; source schedule upgrades, committed wave, blocked occupancy, legacy and removal.',
 exclusions:['Long natural games','Cartesian fog/join/seed/size products','Rendering stress','Map generation and wave spawning mechanics covered by earlier tasks; source transition fixtures do not claim observed network waves'],
 commands:['node20 ai/test-task240-source.js <run>','node20 ai/test-task240-browser.js <run>','git diff --check','git diff --cached --name-only','source and evidence audit including cleanup']});
...
try {
  ...
  command(process.execPath,[path.join(__dirname,'test-task240-source.js'),out]);
  checkpoints.push(...read('source-checkpoints.json').checkpoints);
  command(process.execPath,[path.join(__dirname,'test-task240-browser.js'),out]);
  ...
  assert.equal(browser.rows.length,13);
  const render=read('ui-state.json').rows;assert.deepEqual(render,browser.rows);
  assert.deepEqual(render.map(r=>r.id).sort(),[...browserCases.flatMap(id=>categories.map(c=>id+'/'+c)),'browser-simultaneous-fog-true/committed-upgrade'].sort());
  ...
  write('ui-state.json',{...read('ui-state.json'),checkpoints,pass:true});
  ...
  log(`PASS evidence-audit cases=${cases.length} checkpoints=${checkpoints.length} categories=6 stats-back-reselection=pass captures=13 inspection-mutations=0`);
  ...
  write('coverage-results.json',{cases:cases.map(id=>({id,pass:passed,proof:['checkpoints.json','ui-state.json']})),
