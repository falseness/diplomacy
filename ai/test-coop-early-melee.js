...
function run(fault) {
  ...
  console.log('PASS co-op early melee types=3 movement_limits=2,3,4 damage=1,1,2 lethal=3 nonlethal=10 persistence=3');
}
if (require.main === module) {
  if (process.argv[2] === '--fault') run(process.argv[3]);
    ...
    assert.ok(child.stderr.includes('AssertionError') && child.stderr.includes('imp-combat-exact-config-and-identity'));
