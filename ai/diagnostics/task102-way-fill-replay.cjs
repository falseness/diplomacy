...
const observer = require('./task102-way-fill.cjs').install(variant);
async function main() {
  ...
  console.log('WAY_FILL_REPLAY: PASS 12 complete results');
  if (variant === 'audit') { assert(observer.counts.checks > 0); }
  console.log('WAY_FILL_EXECUTION: PASS ' + JSON.stringify(observer.counts));
}
