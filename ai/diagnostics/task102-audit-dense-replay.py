#!/usr/bin/env python3
"""Audit every bounded dense vector replay; no full speed acceptance claim."""
import gzip
import hashlib
import json
...
def audit(directory):
    ...
    for index, variant in enumerate(manifest['order'], 1):
        ...
        assert 'DENSE_REPLAY: PASS 12 complete results' in log, prefix
        assert 'EXIT_CODE: 0\n' in log, prefix
    ...
    print('EXACT_VECTOR_ORACLE: PASS', json.dumps(passes[-1]['counts'], sort_keys=True))
    ...
    (directory / "summary.json").write_text(json.dumps({"promote": promote, "workloads": summary}, indent=2) + "\n")
    return summary
