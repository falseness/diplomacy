#!/usr/bin/python3
"""Fail-closed audit of the saved-boundary intra-op prerequisite, including non-results."""
import hashlib
import json
...
def main():
    ...
    arms = []
    drift = []
    for intra in plan['boundedOrder']:
        run = dest / f'bounded-intra-{intra}'
        if not run.exists():
            print(f'UNRUN ARM: intra={intra}; prerequisite not passed')
            continue
        ...
        for b, expected in zip(report['boundaries'], plan['boundaries']):
            ...
            for actual, old in zip(b['results'], expected['results']):
                ...
                if differences: drift.append(dict(intra=intra, seed=actual['seed'], differences=differences))
        ...
        for e in launch:
            assert e['intra'] == str(intra) and e['inter'] == '2' and e['v8'] == 4 and e['affinity'] == [0, 1]
        ...
        for e in initialized:
            assert e['intra'] == str(intra) and e['inter'] == '2'
        ...
        arm = dict(intra=intra, boundaries=len(report['boundaries']), attempted=len(starts), completed=len(results),
        ...
        arms.append(arm)
        print(f'NATIVE CONFIG: PASS intra={intra} inter=2 v8=4 shared=0,1 initializedThreads={list(counts.values())}')
        print(f'LIFECYCLE: PASS intra={intra} boundaries={arm["boundaries"]} stableTensors={tensors} ownedTensors=0 reaped=2')
        print(f'OUTCOMES: intra={intra} attempted={arm["attempted"]} completed={arm["completed"]} nonResults={arm["nonResults"]}')
    ...
    print('NATIVE POOL PARITY: '+('PASS boundariesPerArm=2 gamesPerArm=4' if passed else 'FAIL; no CLI screen authorized'))
