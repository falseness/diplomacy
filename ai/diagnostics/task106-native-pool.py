#!/usr/bin/python3
"""One-factor evaluator native pool prerequisite. Rejected arms are never retried."""
...
def child():
    ...
    intra = os.environ['TASK106_EVALUATOR_INTRA']
    assert intra in ('1', '2')
    ...
    with open(os.environ['TASK106_AFFINITY_EVENTS'], 'a') as log:
        log.write(json.dumps(dict(event='before-node', pid=os.getpid(),
            index=os.environ['TASK106_CHILD_INDEX'], affinity=[0, 1], intra=intra,
            inter='2', v8=4, ns=str(time.monotonic_ns()))) + '\n')
...
def main():
    ...
    for iteration, names in [(25, ['decision.md', 'quality-semantic-audit.json', 'quality-audit.log']),
        ...
        (22, ['worker-invariants.log']), (23, ['cli-final.log'])]:
    ...
    plan = dict(head=subprocess.check_output(['git', 'rev-parse', 'HEAD'], text=True).strip(),
        ...
        intra='only treatment: evaluator 2 versus 1', inter=2, v8=4, cpus=[0, 1], heapMiB=6144,
    ...
    for intra in plan['boundedOrder']:
        ...
        env = dict(os.environ, NODE_PATH='/usr/share/nodejs',
            ...
            DIPLOMACY_TASK104_DETERMINISTIC_INVARIANT='1', TASK106_EVALUATOR_INTRA=str(intra),
        ...
        print('COMMAND:', json.dumps(cmd), 'INTRA:', intra, flush=True)
        ...
        print(f'BOUNDED EXIT: intra={intra} exit={process.returncode} seconds={elapsed}', flush=True)
        if process.returncode:
            print('NATIVE POOL PREREQUISITE: FAIL; stop without CLI timing or retries', flush=True)
            return 1
    print('NATIVE POOL PREREQUISITE: PASS; audit effective settings before cold screen', flush=True)
    return 0
