#!/usr/bin/env python3
"""Audit every bounded queue replay; no full speed acceptance claim."""
import gzip
import hashlib
import json
...
def audit(directory):
    ...
    for index, variant in enumerate(manifest['order'], 1):
        ...
        assert 'QUEUE_REPLAY: PASS 12 complete results' in log, prefix
        assert 'EXIT_CODE: 0\n' in log, prefix
    ...
    print('TRAVERSAL_DISTANCE_PARENT_BORDER: PASS', json.dumps(passes[-1]['counts'], sort_keys=True))
