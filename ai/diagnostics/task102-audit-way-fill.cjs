...
const {transform, filename} = require('./task102-way-fill.cjs');
...
for (let i = 0; i < labels.length; i++) {
  ...
  assert.match(log, /^WAY_FILL_REPLAY: PASS 12 complete results$/m);
  ...
  for (let j = 0; j < record.scripts.length; j++) {
    ...
    assert.equal(script.sha256, script.filename === filename
...
console.log('WAY_FILL_ORACLE: PASS ' + JSON.stringify(reference.counts));
