...
const observer = require('./task102-sham-wrapper.cjs').install(variant);
async function main() {
  ...
  assert.equal(observer.counts.replacements, variant === 'S' ? 1 : 0);
  assert.equal(observer.counts.forwards, variant === 'S' ? observer.scripts.reduce((sum, r) => sum + r.executions, 0) : 0);
  console.log('SHAM_REPLAY: PASS 12 complete results');
  console.log('SHAM_EXECUTION: PASS ' + JSON.stringify(observer.counts));
}
