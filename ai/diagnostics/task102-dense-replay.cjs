...
const counts = variant === 'control' ? {} :
  require('./task102-dense-vectors.cjs').install(variant === 'audit');
async function main() {
    ...
    for (let index = 0; index < 6; index += 1) {
      ...
      const result = group === 'teacher'
        ...
        : runGame({...manifest.component[index],
          ...
          inferenceSource: 'TASK-102 bounded dense vectors diagnostic frozen checkpoint'});
  ...
  console.log('DENSE_REPLAY: PASS 12 complete results');
  console.log('DENSE_ORACLE: ' + JSON.stringify(counts));
}
