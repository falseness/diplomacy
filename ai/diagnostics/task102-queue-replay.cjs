...
const counts = variant === 'control' ? {} :
  require('./task102-queue-cursor.cjs').install(variant === 'audit');
async function main() {
    ...
    for (let index = 0; index < 6; index += 1) {
      ...
      const result = group === 'teacher'
        ...
        : runGame({...manifest.component[index],
          ...
          inferenceSource: 'TASK-102 bounded queue cursor diagnostic frozen checkpoint'});
  ...
  console.log('QUEUE_REPLAY: PASS 12 complete results');
  console.log('QUEUE_EQUIVALENCE: ' + JSON.stringify(counts));
}
