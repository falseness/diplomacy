...
const out = path.join(root, 'artifacts/TASK-034');
...
(async () => {
  ...
  try {
    ...
    compare('distinct-silhouettes-without-labels',new Set(symbols.map(s=>s.png)).size,10);
    await check('all-ten');
    ...
    console.log('PASS co-op late render types=10 distinct_symbols=10 checkpoints=11 invariant_checkpoints=11');
  } finally {
