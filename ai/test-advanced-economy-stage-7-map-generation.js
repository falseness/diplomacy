...
function assertNoInvalidObjects(map, seed) {
  assert((map.goldmines || []).length === 0,
    'advanced stage 7 should not generate goldmines', { seed, goldmines: map.goldmines });
  assert((map.lakes || []).length === 0,
    'advanced stage 7 should not generate lakes', { seed, lakes: map.lakes });
  assert((map.mountains || []).length === 0,
    'advanced stage 7 should not generate mountains', { seed, mountains: map.mountains });
  assert((map.bushes || []).length === 0,
    'advanced stage 7 should not generate bushes', { seed, bushes: map.bushes });
  ...
  for (let playerIndex = 0; playerIndex < map.players.length; playerIndex += 1) {
    ...
    for (const field of [
      ...
      assert(objectCount(player, field) === 0,
        'advanced stage 7 should not generate player ' + field,
...
function validateSuburbs(map, seed) {
  ...
  for (let playerIndex = 1; playerIndex <= 2; playerIndex += 1) {
    ...
    assert(player.suburbs.length === 1,
      'advanced stage 7 should configure one suburb layout per player', { seed, playerIndex });
    const layout = player.suburbs[0];
    const town = player.towns[0];
    assert(key(layout.town) === key(town),
      'advanced stage 7 suburb layout references the wrong town', { seed, playerIndex, layout });
    assert(layout.cells.length >= 2,
      'advanced stage 7 should capture at least one suburb cell per player',
    ...
    for (const cell of layout.cells) {
      assert(cell.x >= 0 && cell.y >= 0 && cell.x < 9 && cell.y < 9,
        'advanced stage 7 suburb cell is outside the 9x9 map', { seed, playerIndex, cell });
      assert(!claimed.has(key(cell)) || key(cell) === key(town),
        'advanced stage 7 suburb cell is claimed by multiple players',
      ...
      if (key(cell) !== key(town)) {
        assert(!townKeys.has(key(cell)),
          'advanced stage 7 captured suburb overlaps a town', { seed, playerIndex, cell });
        assert(layout.cells.some((other) =>
          connected[key(other)] && distance(other, cell) === 1),
        'advanced stage 7 captured suburb is disconnected from its town',
...
function validateMap(map, seed) {
  assert(map.mapSize.x === 9 && map.mapSize.y === 9,
    'advanced stage 7 must be a 9x9 map', { seed, mapSize: map.mapSize });
  assert(map.players.length === 3,
    'advanced stage 7 should include neutral plus two players', { seed });
  assert(map.advancedEconomyStage === 7,
    'advanced stage 7 metadata is missing', { seed, advancedEconomyStage: map.advancedEconomyStage });
  assert(map.economyGenerator && map.economyGenerator.stage === 'advanced-7',
    'advanced stage 7 generator metadata is missing', { seed, economyGenerator: map.economyGenerator });
  ...
  assert(map.players[1].towns.length === 1 && map.players[2].towns.length === 1,
    'advanced stage 7 should contain exactly one red town and one blue town',
  ...
  assert(redTown.y + blueTown.y === 8 && redTown.x === blueTown.x,
    'advanced stage 7 towns should remain mirrored across the center lane',
    { seed, redTown, blueTown });
  assert(map.players[1].gold === map.players[2].gold,
    'advanced stage 7 should not give either player extra starting gold',
  ...
  assert(map.economyObjects && map.economyObjects.towns === 2 &&
    ...
    'advanced stage 7 economy object summary is incorrect',
    { seed, economyObjects: map.economyObjects });
}

const { context } = loadAiScripts();
const api = new Function('context', `return {
  generateAdvancedEconomyStage7TrainingMap: context.generateAdvancedEconomyStage7TrainingMap
};`)(context);

assert(typeof api.generateAdvancedEconomyStage7TrainingMap === 'function',
  'generateAdvancedEconomyStage7TrainingMap must be exported to the AI script context');

const observedLayouts = new Set();
const observedCapturedCounts = new Set();
for (let seed = 14400; seed < 14560; seed += 1) {
  const map = api.generateAdvancedEconomyStage7TrainingMap({ seed });
  ...
  const repeated = api.generateAdvancedEconomyStage7TrainingMap({ seed });
  assert(normalizeMap(map) === normalizeMap(repeated),
    'advanced stage 7 map generation is not deterministic for seed ' + seed);
  ...
  for (let playerIndex = 1; playerIndex <= 2; playerIndex += 1) {
    ...
    assert(town.suburbs.length === generatedLayout.cells.length,
      'advanced stage 7 runtime lost configured suburb cells',
    ...
    'advanced stage 7 runtime suburb ownership does not match its town',
    { seed, playerIndex, suburbs: town.suburbs });
  }
}

assert(observedLayouts.size > 1,
  'advanced stage 7 fixed-seed sample did not exercise suburb layout variation',
  { observedLayouts: Array.from(observedLayouts).slice(0, 5) });
assert(observedCapturedCounts.size > 1,
  'advanced stage 7 fixed-seed sample did not exercise captured suburb count variation',
  { observedCapturedCounts: Array.from(observedCapturedCounts) });

async function runGameplaySmoke() {
  const smokeMap = api.generateAdvancedEconomyStage7TrainingMap({
    seed: 14442,
    suddenDeathRound: 12
  });
  ...
  console.log('Advanced economy stage 7 9x9 captured-suburb map generation smoke passed');
}
