...
function feasible(h,size,n,perFront) {
 ...
 for(const depth of range(1,Math.floor(n/3))) {
  ...
  const minWidth=range(1,Math.floor(n/2)).find(w=>count(0,w)>=perFront*h&&count(n-w,w)>=perFront*h);
...
const cache=new Map();
function expectation(h,size,perFront=3){
 const key=size+':'+h+':'+perFront;if(cache.has(key))return cache.get(key);
 ...
 while(!(witness=feasible(h,size,side,perFront))){rejected.push(side++);if(side>200)throw Error('oracle bound');}
