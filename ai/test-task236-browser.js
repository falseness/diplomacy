...
async function main(){
 ...
 const check=(id,observed,expected)=>{const pass=JSON.stringify(observed)===JSON.stringify(expected);checks.push({id,observed,expected,pass});write('browser-checkpoints.json',{checkpoints:checks});assert.deepEqual(observed,expected,id);console.log('PASS '+id);};
 for(const c of cases){
  ...
  try{
   assert(Number(process.env.TASK236_STOP_AT)-Date.now()>300000,'reserve service cleanup budget');
   const result=await withServices({evidenceDir:dir,bounds:{scenarioMs:240000}},async service=>{
    ...
    const portals=Array.from({length:12},(_,i)=>({x:i%11,y:10-Math.floor(i/11),category:categories[i%6]}));
    const spec=currentCoopFixtureSpec({label:c.id,humans:2,size:'tiny',seed:1,portals,
     purpose:'TASK-236 six-category transport/browser fixture; generated enlargement validation belongs to TASK-237',
     players:[{rgb:{r:100,g:100,b:100},gold:0,towns:[]},{rgb:{r:255,g:0,b:0},gold:100,towns:[{x:2,y:3}]},{rgb:{r:0,g:0,b:255},gold:100,towns:[{x:8,y:3}]}]});
    ...
    const want={side:[11,11],fog:c.fog,initialHumans:2,portals:portals.map(p=>({x:p.x,y:p.y,category:p.category})).sort((a,b)=>a.x-b.x||a.y-b.y)};
    check(c.id+'/fixture-counts',categories.map(k=>want.portals.filter(p=>p.category===k).length),[2,2,2,2,2,2]);
    const joined=[];
    ...
    for(let i=0;i<2;i++){
     ...
     check(c.id+'/render-assets-'+i,render.map(e=>e.loaded&&e.cached),Array(12).fill(true));
     const frame=await p.observe(()=>lastGameFrameTime);await p.page.waitForFunction(v=>lastGameFrameTime>v,frame);
     write(c.id+'/render-assets-'+i+'.json',render);
     const shot=await p.screenshot('six-category-fixture');
     rows.push({id:c.id,player:i,expected:want,observed:project(actual),screenshot:c.id+'/screenshots/'+shot.file});
    }
